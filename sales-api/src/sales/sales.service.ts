...
export class SalesService {
  ...
  async replaceItems(
    ...
    const sale = await this.prisma.sale.findUnique({
      where: { id: saleId },
      select: { id: true, sellerId: true, paymentStatus: true },
    });
    ...
    if (sale.paymentStatus === PaymentStatus.PAID) {
      throw new BadRequestException(
        'Não é possível alterar itens de uma venda já paga',
      );
    }
  ...
  async getStatusCounts(params: {
    ...
    const counts = {
      total: sales.length,
      paid: sales.filter((sale) => sale.paymentStatus === PaymentStatus.PAID)
        .length,
      pending: sales.filter(
        (sale) => sale.paymentStatus === PaymentStatus.PENDING,
      ).length,
      canceled: sales.filter(
        (sale) => sale.paymentStatus === PaymentStatus.CANCELED,
      ).length,
    };
