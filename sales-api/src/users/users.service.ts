...
const SAFE_USER_SELECT = {
    ...
    commissionPercent: true, lastLoginAt: true, createdAt: true, updatedAt: true,
};
...
export class UsersService {
    ...
    // ---------- SELLERS ----------
    async createSeller(data: { name: string; email: string; password: string; commissionPercent?: string }) {
    ...
    async updateSeller(id: string, data: { name?: string; email?: string; password?: string; commissionPercent?: string }) {
        const toUpdate: any = {};
        if (data.name) toUpdate.name = data.name;
        if (data.email) {
            // checa duplicidade
            const exists = await this.prisma.user.findUnique({ where: { email: data.email } });
            if (exists && exists.id !== id) throw new ConflictException('E-mail already in use');
