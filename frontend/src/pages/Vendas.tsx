...
import { PAYMENT_STATUS_LABELS, PAYMENT_STATUS_COLORS, PAYMENT_METHOD_LABELS } from '../types/sales.types'
...
const Vendas = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            header: 'VENDEDOR',
            render: (sale: Sale) => (
                <div className="text-secondary-900">{sale.seller.name}</div>
            )
        },
        {
            key: 'date',
    ...
    const loadSales = useCallback(async (queryFilters: SalesFilters) => {
        try {
            ...
            const query: SalesQuery = {
                ...
                paymentStatus: queryFilters.paymentStatus === 'ALL' ? undefined : queryFilters.paymentStatus
            }
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-2 mt-1">
                                ...
                                {filters.paymentStatus !== 'ALL' && (
                                    <span className="px-2 py-1 bg-info-100 text-info-800 text-xs rounded-full">
                                        Status: {PAYMENT_STATUS_LABELS[filters.paymentStatus as keyof typeof PAYMENT_STATUS_LABELS]}
                                    </span>
                                )}
                            </div>
                        </div>
