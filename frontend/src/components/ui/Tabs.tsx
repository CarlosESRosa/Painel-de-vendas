...
const Tabs = ({ tabs, activeTab, onTabChange, variant = 'default', className = '' }: TabsProps) => {
  const getTabClasses = (tab: Tab, isActive: boolean, index: number) => {
    const baseClasses =
      'px-4 py-2 text-sm font-medium transition-all duration-200 cursor-pointer flex items-center space-x-2';
  ...
  if (variant === 'pills') {
    return (
      <div
        className={`inline-flex rounded-lg border border-secondary-300 bg-white shadow-sm ${className}`}
      >
        {tabs.map((tab, index) => (
          <div
            key={tab.id}
          ...
          >
            ...
            {tab.count !== undefined && (
              <span
                className={`px-2 py-0.5 text-xs font-medium rounded-full border ${getColorClasses(
                  tab.color,
                )}`}
              >
                {tab.count}
